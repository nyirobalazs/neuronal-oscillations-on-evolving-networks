...
save(sprintf('RESULTS-Beta=%s-FixedWithSelfWeightsEta.mat', num2str(0.25)), 'RESULTS', '-v7.3')
...
saveas(gcf,'Fig1-FixedWithSelfWeightsEta.png')
...
saveas(gcf,'Fig2-FixedWithSelfWeightsEta.png')
...
niter = 12; %e number of iterations

betalevel = beta; % dummy

W_OUT = RESULTS{1,1}(:,(2*N)+1:2*N+N^2); % get all the network data across all 30 years (1 row for each year)

A_VECTORS = zeros(niter, tspan(end), N,4); % initialise matrix to store all A values (1 column for each year and 1 row for each iteration, third dimension is for each node)
sols = cell(niter, tspan(end),4); % store the ODE solutions here (1 column for each year and 1 row for each iteration)

A_ALLs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
A_FRONTSs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
A_PARIs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
A_OCCs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
A_TEMPs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
A_LIMBs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
A_BASGs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
A_BSs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)

Bs = zeros(niter, tspan(end),4); % initialise matrix to store B values (1 column for each year and 1 row for each iteration)

B_ALLs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
B_FRONTSs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
B_PARIs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
B_OCCs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
B_TEMPs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
B_LIMBs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
B_BASGs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)
B_BSs = zeros(niter, tspan(end),4); % initialise matrix to store A values (1 column for each year and 1 row for each iteration)

eps_range = [0, 0.33, 0.66, 1];

W_matrix_reweighted = cell(1,30);

len = length(eps_range);

tic % time the code
for eps_level = 1:len
    eps = eps_range(eps_level);
    
    for T = tspan
        W_matrix = sparse(reshape(W_OUT(T,:), N, N)); % turn network into matrix
        if (T > 1)
            W_matrix_reweighted_minus1 = W_matrix_reweighted(1,T-1);
            W_matrix_bar = sum(W_matrix)/(N^2);
            W_matrix_minus1_bar = sum(W_matrix_reweighted_minus1{1,1})/(N^2);
            W_matrix = ((1-eps) + eps*(W_matrix_minus1_bar/W_matrix_bar)).*W_matrix;
        end
        W_matrix_reweighted{1,T} = W_matrix;
    end

    for iter = 1:niter
        parfor (T = tspan, 8)
            fprintf('Eps = %s, Iter = %s, Year = %s\n', num2str(eps), num2str(iter), num2str(T));
    
            W_matrix = W_matrix_reweighted(1,T);
            ...
            A_VECTORS(iter, T, :, eps_level) = A;
            A_ALLs(iter, T, eps_level) = A_ALL;
            A_FRONTs(iter, T, eps_level) = A_FRONT;
            A_PARIs(iter, T, eps_level) = A_PARI;
            A_OCCs(iter, T, eps_level) = A_OCC;
            A_TEMPs(iter, T, eps_level) = A_TEMP;
            A_LIMBs(iter, T, eps_level) = A_LIMB;
            A_BASGs(iter, T, eps_level) = A_BASG;
            A_BSs(iter, T, eps_level) = A_BS;
            ...
            B_VECTORS(iter, T, :, eps_level) = B;
            B_ALLs(iter, T, eps_level) = B_ALL;
            B_FRONTs(iter, T, eps_level) = B_FRONT;
            B_PARIs(iter, T, eps_level) = B_PARI;
            B_OCCs(iter, T, eps_level) = B_OCC;
            B_TEMPs(iter, T, eps_level) = B_TEMP;
            B_LIMBs(iter, T, eps_level) = B_LIMB;
            B_BASGs(iter, T, eps_level) = B_BASG;
            B_BSs(iter, T, eps_level) = B_BS;
    
        end
    
        % Save available results after each iterations in case it crashes
    
        save(sprintf('A-VECTOR-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.mat', num2str(eps), num2str(betalevel)), 'A_VECTORS')
        save(sprintf('B-VECTOR-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.mat', num2str(eps), num2str(betalevel)), 'B_VECTORS')
    
        writematrix(A_ALLs,sprintf('As-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.csv', num2str(eps), num2str(betalevel)))
        writematrix(B_ALLs,sprintf('Bs-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.csv', num2str(eps), num2str(betalevel)))
        ...
        if (iter > 1)
            ...
            for A_MATRIX = {A_ALLs, A_FRONTs, A_PARIs, A_OCCs, A_TEMPs, A_LIMBs, A_BASGs}
                ...
                A_MATRIX_NONZERO = A_MATRIX_NONZERO(1:iter,:,eps_level);
            ...
            saveas(gcf,sprintf('AT-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.png',num2str(eps),num2str(betalevel)))
            ...
            saveas(gcf,sprintf('AT-Normed-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.png',num2str(eps),num2str(betalevel)))
            ...
            for B_MATRIX = {B_ALLs, B_FRONTs, B_PARIs, B_OCCs, B_TEMPs, B_LIMBs, B_BASGs}
                ...
                B_mean = mean(B_MATRIX_NONZERO(1:iter,:,eps_level), 1);
                B_sd = std(B_MATRIX_NONZERO(1:iter,:,eps_level), 1);
                if thick
            ...
            saveas(gcf,sprintf('BT-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.png',num2str(eps),num2str(betalevel)))
            ...
            for B_MATRIX = {B_ALLs, B_FRONTs, B_PARIs, B_OCCs, B_TEMPs, B_LIMBs, B_BASGs}
                ...
                B_MATRIX_NONZERO = B_MATRIX{1};
                B_MATRIX_NONZERO = B_MATRIX_NONZERO(1:iter,:,eps_level);
            ...
            saveas(gcf,sprintf('BT-Normed-Eps=%s-Beta=%s-FixedWithSelfWeightsEta.png',num2str(eps),num2str(betalevel)))
...
lightBLUE = [174,214,245]/255;
darkBLUE = [43,130,252]/255;
colors_BLUE = [linspace(lightBLUE(1),darkBLUE(1),len)', linspace(lightBLUE(2),darkBLUE(2),len)', linspace(lightBLUE(3),darkBLUE(3),len)'];

lightGRAY = [0.5,0.5,0.5];
darkGRAY = [0,0,0];
colors_GRAY = [linspace(lightGRAY(1),darkGRAY(1),len)', linspace(lightGRAY(2),darkGRAY(2),len)', linspace(lightGRAY(3),darkGRAY(3),len)'];
...
hold on
for eps_level = 1:len
    A_MATRIX_NONZERO = A_VECTORS(:,:,:,eps_level);

    A_MATRIX_NONZERO = mean(A_MATRIX_NONZERO,3);
    initMean = mean(A_MATRIX_NONZERO(:,1));
    A_MATRIX_NONZERO = A_MATRIX_NONZERO/initMean;
    ...
    errorbar(1:30, A_mean, A_sd, 'Color', colors_GRAY(eps_level,:),'LineWidth',3)
...
for eps_level = 1:len
    ...
    A_MATRIX_NONZERO = mean(A_MATRIX_NONZERO(:,:,temporal),3);
    initMean = mean(A_MATRIX_NONZERO(:,1));
    A_MATRIX_NONZERO = A_MATRIX_NONZERO/initMean;
    ...
    errorbar(1:30, A_mean, A_sd, 'Color', colors_BLUE(eps_level,:),'LineWidth',2)


end

ylim([0 1.5])
leg1 = legend('$\xi$ (full) = 0','$\xi$ (full) = 0.33','$\xi$ (full) = 0.66','$\xi$ (full) = 1', ...
    '$\xi$ (temporal) = 0','$\xi$ (temporal) = 0.33','$\xi$ (temporal) = 0.66','$\xi$ (temporal) = 1');
set(leg1,'Interpreter','latex');
title(sprintf('Evolution of average oscillatory activity over time, varying homeostasis'))
xlabel('Time T (yr)')
ylabel('A(T), A_s(T)')

saveas(gcf,sprintf('AT-normed-vs-Eta2.png'))

%save(sprintf('sols-Beta=%s-FixedWithSelfWeightsEta.mat', num2str(betalevel)), 'sols', '-v7.3')
